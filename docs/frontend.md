...
- `app/providers.tsx` — client component wrapping `(main)` layout children with a global `SWRConfig` (`revalidateOnFocus: false`, `dedupingInterval: 30_000`). Prevents refetch thrash when switching browser tabs; per-page `useSWR` keys are unchanged.
  **Invalidating another page's key before navigating to it needs the three-argument `mutate`**: `mutate(key, undefined, { revalidate: true })`, not `mutate(key)`. SWR only fetches on `mutate` when a mounted component subscribes to that key, and the page you are about to push to is by definition not mounted yet — so the bare form finds no revalidator, returns the stale value, and leaves it in cache. The destination then mounts, reads that cached value, reports `isLoading: false`, and paints the previous data until its own refetch lands. Passing `undefined` as data clears the entry outright; `startRevalidate` still runs afterwards and drops the dedupe markers, so `dedupingInterval` is not what is at stake here. Verified against the real SWR module, since the page tests mock `swr` and only prove the call shape.
